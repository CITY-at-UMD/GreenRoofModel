...
// EnergyPlus Headers
#include <EcoRoofManager_modified.hh>
...
namespace EcoRoofManager {
	...
	//***--------
		...
		Real64 f_VMC;  //Empirical multiplicative functions for Substrate Volumetric Water Content
...
//***------------
		...
		static Real64 VMC_fc;  // Substrate volumetric water content at field capacity
...
//---Soil albedo
           ...
           Alphag = 0.2171 * (Mg)**2 - 0.4336 * Mg + 0.3143;
...
//---h_por 
        ...
        NU_por = 1.128*Pe**0.5;               //Nusselt number for porous media
...
//---Newton's method for solving T_plant
        ...
        //Assuming that the sky emissivity is equal to the plant emissivity
               Q_IR_sky_p = ( 1-tau_lw ) * epsilonp * Sigma * ( Surface( SurfNum ).ViewFactorSky * ( SkyTempKelvin )**4 - Tp_old**4 - ( 1-epsilonp ) * Surface( SurfNum ).ViewFactorSky * ( SkyTempKelvin )**4);
               Q_IR_exch_p= ( 1-tau_lw) * Sigma * epsilonp * epsilong * ( T_soil**4 - Tp_old**4) / EpsilonOne;
               Qconv_p = LAI * h_conv( SurfNum,Tak,Tp_old,WS,k_air ) * ( Tp_old - Tak );
               r_a = Rhoa * Cp_air * ( Le_num )**( 2/3 ) / h_conv( SurfNum,Tak,Tp_old,WS,k_air );  //Aerodynamic resistance to 
                                                                                                  //mass transfer, s/m
               ...
               Var_2 = 0.0016 * ( 35.0 - Tp_old + KelvinConv )**2 - 1.0;
               Var_a = ( LAI * Rhoa * Cp_air / gamma_s( T_soil,Cp_air,Pa ));
               Var_b = ( StomatalResistanceMin / LAI ) * f_solar * f_Hum( Tp_old,eair ) * F_VWC;

               Func_prim_p = -4.0 * ( 1.0-tau_lw ) * epsilonp * Sigma * Tp_old**3 - 4.0 * ( 1-tau_lw ) * Sigma * epsilonp * epsilong * Tp_old**3 / EpsilonOne - LAI * h_conv( SurfNum,Tak,Tp_old,WS,k_air ) - ( Var_a * 0.6108 * Var_1 * ( 17.27 / ( Tp_old-KelvinConv + 237.3 ) - 17.27 * ( Tp_old - KelvinConv ) / ( Tp_old - KelvinConv + 237.3)**2) / ( r_a + Var_b / abs(Var_2))) + Var_a * Var_b * 0.0016 * copysign(Var_2,1.0) * ( eair - 0.6108 * Var_1 ) * ( 2.0 * 35.0 - 2.0 * Tp_old + 2.0 * KelvinConv ) / ( abs(Var_2)**2 * ( r_a + Var_b / abs(Var_2))**2);
         ...
         // Bisection Method:
          ...
          //Assuming that the sky emissivity is equal to the plant emissivity
                        Q_IR_sky_p = ( 1-tau_lw ) * epsilonp * Sigma * ( Surface( SurfNum ).ViewFactorSky * ( SkyTempKelvin )**4 - MidPoint**4 - ( 1-epsilonp ) * Surface( SurfNum ).ViewFactorSky * ( SkyTempKelvin )**4);
                        Q_IR_exch_p = ( 1-tau_lw ) * Sigma * epsilonp * epsilong * ( T_soil**4 - MidPoint**4 ) / EpsilonOne;
                        Qconv_p = LAI * h_conv(SurfNum,Tak,MidPoint,WS,k_air) * ( MidPoint - Tak );
                        r_a = Rhoa * Cp_air * (Le_num)**(2/3) / h_conv(SurfNum,Tak,MidPoint,WS,k_air);
            ...
            r_a = Rhoa * Cp_air * (Le_num)**(2/3) / h_conv( SurfNum, Tak, T_plant, WS, k_air );
...
//---Newton's method for solving T_soil covered by plants	
        ...
        if( sigma_f != 0.0) {
           ...
           do {
              ...
              Q_IR_sky_s = tau_lw * epsilong * Sigma * (Surface( SurfNum ).ViewFactorSky * ( SkyTempKelvin )**4 - Ts_old**4 - ( 1 - epsilong ) * Surface( SurfNum ).ViewFactorSky * ( SkyTempKelvin )**4);
              Q_IR_exch_s = ( 1 - tau_lw ) * Sigma * epsilonp * epsilong * ( T_plant**4 - Ts_old**4) / EpsilonOne;
              Qconv_s = ( h_por * h_conv( SurfNum, Tak, T_plant, WS, k_air ) / ( h_por + h_conv( SurfNum, Tak, T_plant, WS, k_air )))*( Ts_old - Tak );

              r_a_sub = Rhoa * Cp_air * (Le_num)**(2/3) * ( 1 / h_por + 1 / h_conv( SurfNum, Tak, T_plant, WS, k_air ));
              ...
              if ( Q_E_s == 0.0) {
                   ...
                   Q_E_S_prim = (( Rhoa * Cp_air / ( r_s_sub + r_a_sub )) * 0.6108 * 0.622 * 1000.0**2 * std::exp(-( 17.27 * ( KelvinConv - Ts_old )) / ( Ts_old - KelvinConv + 237.3 )) * ( 17.27 / ( Ts_old - KelvinConv + 237.3 ) + ( 17.27 * ( KelvinConv - Ts_old )) / ( Ts_old - KelvinConv + 237.3)**2) * ( 2501.1 - (-2.3793) * ( KelvinConv - Ts_old ))) / ( Cp_air * Pa ) - (( Rhoa * Cp_air / ( r_s_sub + r_a_sub )) * 0.622 * (-2.3793) * 1000.0**2 * ( eair - 0.6108 * std::exp(-(17.27 * ( KelvinConv - Ts_old )) / ( Ts_old - KelvinConv + 237.3 )))) / ( Cp_air * Pa );
              }
              Func_prim_s = -4.0 * Sigma * Ts_old**3 * epsilong * tau_lw + ( 4.0 * Sigma * Ts_old**3 * epsilong * epsilonp * ( tau_lw - 1.0 )) / EpsilonOne - ( h_conv( SurfNum, Tak, T_plant, WS, k_air ) * h_por ) / ( h_conv( SurfNum, Tak, T_plant, WS, k_air ) + h_por ) - Q_E_S_prim - Qsoilpart2 * sigma_f;
              ...
              if (iter2 >= 100) {   // Means if Newton's Method entered an infinite loop use Bisection method instead
                ...
                // Bisection Method:
                      ...
                      //Assuming that the sky emissivity is equal to the soil emissivity
                      Q_IR_sky_s  = tau_lw * epsilong * Sigma * (Surface(SurfNum).ViewFactorSky * (SkyTempKelvin)**4 - MidPoint**4 - ( 1 - epsilong ) * Surface(SurfNum).ViewFactorSky * (SkyTempKelvin)**4);
                      Q_IR_exch_s = ( 1- tau_lw ) * Sigma * epsilonp * epsilong * ( T_plant**4 - MidPoint**4 ) / EpsilonOne;
                      ...
                      else (((Func_MidPoint < 0.0) && (Func_2 > 0.0)) || ((Func_MidPoint > 0.0) && (Func_2 < 0.0))) {
           ...
           Q_IR_s_Rep = tau_lw * epsilong * Sigma * ( Surface(SurfNum).ViewFactorSky * (SkyTempKelvin)**4 - T_soil**4 - ( 1 - epsilong ) * Surface(SurfNum).ViewFactorSky * (SkyTempKelvin)**4) + ( 1 - tau_lw ) * Sigma * epsilonp * epsilong * ( T_plant**4 - T_soil**4 ) / EpsilonOne;
		}
    ...
    if (sigma_f != 1) {
       ...
       iter3 = 0;
        ...
        //Assuming that the sky emissivity is equal to the bare soil emissivity
           Q_IR_sky_bare_s = epsilong * Sigma * (Surface(SurfNum).ViewFactorSky * (SkyTempKelvin)**4 - Ts_bare_old**4 - (1 - epsilong) * Surface(SurfNum).ViewFactorSky * (SkyTempKelvin)**4);
           Qconv_bare_s = h_conv_bare( SurfNum, Tak, Ts_bare_old, WS, k_air ) * ( Ts_bare_old - Tak );
           r_a_bare = Rhoa * Cp_air * (Le_num)**(2/3) / h_conv_bare( SurfNum, Tak, Ts_bare_old, WS, k_air );
           ...
           Q_E_bare_s_prim = (( Rhoa * Cp_air / ( r_s_sub + r_a_bare )) * 0.6108 * 0.622 * 1000**2 * std::exp(-(17.27 * ( KelvinConv - Ts_bare_old )) / ( Ts_bare_old - KelvinConv + 237.3 )) * ( 17.27 / ( Ts_bare_old - KelvinConv + 237.3 ) + ( 17.27 * ( KelvinConv - Ts_bare_old )) / ( Ts_bare_old - KelvinConv + 237.3 )**2 ) * ( 2501.1 - (-2.3793) * ( KelvinConv - Ts_bare_old ))) / ( Cp_air * Pa ) - (( Rhoa * Cp_air / ( r_s_sub + r_a_bare )) * 0.622 * (-2.3793) * 1000**2 * ( eair - 0.6108 * std::exp(-(17.27 * ( KelvinConv - Ts_bare_old )) / ( Ts_bare_old - KelvinConv + 237.3 )))) / ( Cp_air * Pa );
           Func_prim_bare_s = -4.0 * Sigma * Ts_bare_old**3 * epsilong - h_conv_bare( SurfNum, Tak, Ts_bare_old, WS, k_air ) - Q_E_bare_s_prim - Qsoilpart2 * ( 1 - sigma_f );
          ...
          //Assuming that the sky emissivity is equal to the bare soil emissivity
                     Q_IR_sky_bare_s = epsilong * Sigma * (Surface(SurfNum).ViewFactorSky * (SkyTempKelvin)**4 - MidPoint**4 - ( 1 - epsilong ) * Surface(SurfNum).ViewFactorSky * (SkyTempKelvin)**4 );
                     Qconv_bare_s = h_conv_bare( SurfNum, Tak, MidPoint, WS, k_air ) * (MidPoint - Tak);
					 r_a_bare = Rhoa * Cp_air * (Le_num)**(2/3) / h_conv_bare( SurfNum, Tak, MidPoint, WS, k_air );
        ...
        3300:T_bare_soil = Ts_bare_new;
        r_a_bare = Rhoa * Cp_air * (Le_num)**(2/3) / h_conv_bare( SurfNum, Tak, T_bare_soil, WS, k_air );
        ...
        Q_IR_bare_s_Rep = epsilong * Sigma * ( Surface(SurfNum).ViewFactorSky * (SkyTempKelvin)**4 - T_bare_soil**4 - ( 1 - epsilong ) * Surface(SurfNum).ViewFactorSky * (SkyTempKelvin)**4 );
    }
...
//---Convective heat transfer coefficient for plants (h_conv)
    ...
    //IMPLICIT NONE
      ...
      Gr = abs(9.81 * Beta * ( plant_temp - Tair_k ) * L_cha**3/nu_air**2 );
      Re = WindSpeed * length/ nu_air;
     
   //Forced convection
      if(Gr < 0.068 * Re**2.2) {
         Nu = 3.0 + 1.25 * 0.0253 * Re**0.8;
         h_conv = 3.0 * Nu * k_air1/length;
      }
   
   //Mixed convection
      if(Gr > 0.068 * Re**2.2 && Gr < 55.3 * Re**(5/3)) {
         Nu = 2.7 * ( Gr/Re**2.2)**(1/3) * ( 3 * 15/4 + 0.0253 * 15/16 * Re**0.8);
         Norm = ( Gr / Re**(5/3)) / 60;
...
//---Convective heat transfer coefficient for bare soil
    ...
    //IMPLICIT NONE
	  ...
	  {
      Real64 const nu_air(15.66d-6);     // kinematic viscosity (m**2/s) for air at 300 K (Mills 1999 Heat Transfer)
      Real64 const Pr_air(0.71);       // Prandtl number for air
      Real64 Re;         //Reynolds number
      ...
      Gr_soil = abs( 9.81 * Beta * ( BareSoil_temp - Tair_k ) * L_cha**3/nu_air**2 );
      Re = WindSpeed * length/nu_air;

   //Forced convection
      if(Gr_soil < 0.068 * Re**2.2) {
         Nu = 3.0 + 1.25 * 0.0253 * Re**0.8;
         h_conv_bare = 2.1 * Nu * k_air1/length;
      }

   //Mixed convection
      if(Gr_soil > 0.068 * Re**2.2 && Gr_soil < 55.3 * Re**(5/3)) {
         Nu = 2.7 * (Gr_soil/Re**2.2)**(1/3) * ( 3 * 15/4 + 0.0253 * 15/16 * Re**0.8);
         Norm = (Gr_soil/Re**(5/3))/60;
         Lmixed = ( l_cha * Norm + length * (1-Norm));
         h_conv_bare = 2.1 * Nu * k_air1/Lmixed;
       }

   //Natural convection
      if(Gr_soil > 55.3 * Re**(5/3)) {
         Nu = 0.15 * (Gr_soil*Pr_air)**(1/3);
...
//---f_temp    
    ...
    //IMPLICIT NONE
	  ...
	  {
      f_temp = abs(1/(1-0.0016 * (35-(Temperature - KelvinConv))**2));
    return;
	}
